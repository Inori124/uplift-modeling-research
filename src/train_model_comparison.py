...
def main(path,out_path,seed=2027,test_size=.25,class_weight='balanced'):
    ...
    t_models={}
    for label in [0,1]:
        m=make_pipeline(StandardScaler(), LogisticRegression(max_iter=500, class_weight=class_weight, solver='liblinear'))
    ...
    s_model=make_pipeline(StandardScaler(), LogisticRegression(max_iter=500, class_weight=class_weight, solver='liblinear'))
...
if __name__=='__main__':
    ap=argparse.ArgumentParser(); ap.add_argument('path'); ap.add_argument('--out',default='results/model_comparison_seed2027.json'); ap.add_argument('--seed',type=int,default=2027); ap.add_argument('--treatment-ratio',type=float,default=None,help='训练集中 treated/control 抽样比例；默认不抽样'); ap.add_argument('--class-weight',choices=['none','balanced'],default='balanced'); args=ap.parse_args(); main._treatment_ratio=args.treatment_ratio; main(args.path,args.out,args.seed,class_weight=None if args.class_weight=='none' else 'balanced')
