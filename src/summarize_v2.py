...
def summarize(ds,fs):
 r={'n_runs':len(ds),'runs':[{'seed':d['model_seed'],'file':str(f),'train_n':d['train_n'],'treated_fraction':d['train_propensity']} for d,f in zip(ds,fs)],'summary':{}}
 for m in MODELS:
  r['summary'][m]={}
  for metric in METRICS:
   a=np.array([d['metrics'][m][metric] for d in ds],float)
   r['summary'][m][metric]={'mean':float(a.mean()),'std':float(a.std(ddof=1)),'min':float(a.min()),'max':float(a.max())}
 return r

def main(pattern='results/v2_main_seed*.json',out='results/v2_summary.json'):
 fs=sorted(Path('.').glob(pattern)); ds=[json.loads(f.read_text()) for f in fs]
 r=summarize(ds,fs)
 if 'fixed_n120k' in pattern:
  r['requested_fraction']=ds[0].get('requested_treated_fraction')
